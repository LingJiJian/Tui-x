...
bool MenuScene::init()
{
	...
	m_pMenuTableView = CTableView::create(Size(700, 640));
	m_pMenuTableView->setCountOfCell(m_vMenuItems.size());
	m_pMenuTableView->setSizeOfCell(Size(700, 100));
...
Ref* MenuScene::menuTableDataSource(Ref* pConvertCell, unsigned int idx)
{
...
CWidgetTouchModel MenuScene::onTextTouchDown(Ref* pSender, Touch* pTouch)
{
...
bool MenuScene::onTextTouchEnd(Ref* pSender, Touch* pTouch, float fDuration)
{
	onTextTouchCancelled(pSender, pTouch, fDuration);
	return true;
}

bool MenuScene::onTextTouchCancelled(Ref* pSender, Touch* pTouch, float fDuration)
{
	CLabel* pText = (CLabel*) pSender;
	pText->stopActionByTag(1);
...
void MenuScene::onTextClick(Ref* pSender)
{
	CLabel* pText = (CLabel*) pSender;
...
void BaseTestScene::onMenuBtnClick(Ref* pSender)
{
